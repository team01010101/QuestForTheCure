...
public class RobertWalker : WaypointWalker {
  ...
  public override void updateInput() {
    if (!gameManager.robertData.alive) return;
  ...
  public override void awake() {
    gameManager.registerRobert(gameObject);
    ai = GetComponent<RobertAI>();
  }
  ...
  public override void update() {
    ...
    // where's robert facing?
    Debug.DrawLine(transform.position, transform.position + transform.forward.normalized * 4, Color.red);
  }


  //------------------------------------------------------------------------
  // robert moves forward
  //------------------------------------------------------------------------

  private void processMove() {
    isMoving = gameManager.robertData.alive && currentDirection != Direction.None;
    Vector3 moveDir = isMoving ? Vector3.forward : Vector3.zero;
    float speedMultiplier = walkSpeed * gameManager.robertSpeedMultiplier(ai.powerTime);
  ...
  private void plantBobOmb() {
    if (inputBomb) {
      if (gameManager.robertData.bombs > 0) {
        --gameManager.robertData.bombs;
        // spawn in front of robert
  ...
  protected bool validDirection(Direction dir) {
    if (dir == Direction.Front) return currentNode.getFront() != null && currentNode.getFront().robertWalkable;
    else if (dir == Direction.Back) return currentNode.getBack() != null && currentNode.getBack().robertWalkable;
    else if (dir == Direction.Left) return currentNode.getLeft() != null && currentNode.getLeft().robertWalkable;
    else if (dir == Direction.Right) return currentNode.getRight() != null && currentNode.getRight().robertWalkable;
    return false;
  }
